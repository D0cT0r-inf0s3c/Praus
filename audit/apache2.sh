...
then
  status="\e[91m[ BAD ]"
  echo "\e[39m[*] Checking if /etc/apache2/apache2.conf exists\t\t\t\t\t$status\e[39m"
else
...
then
  status="\e[91m[ BAD ]"
  echo  "\e[39m[*] Checking if /etc/apache2/conf-available/security.conf exists\t\t\t$status\e[39m"
else
...
then
  status="\e[91m[ BAD ]"
  echo  "\e[39m[*] Checking if /etc/apache2/mods-available/ssl.conf exists\t\t\t\t$status\e[39m"
else
